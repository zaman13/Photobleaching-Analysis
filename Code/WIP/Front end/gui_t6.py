...
def show_plot():
    img = Image.open('M_15s.normalized_rate.png')
    xx = img.size[0]
    yy = img.size[1]

    demo_media = ttb.PhotoImage(file='M_15s.normalized_rate.png')
    # scl = round((output_frame.winfo_width()/img.size[0]) + 0.5)   # the 0.5 ensures that it'll always round up. Using native round() instead of numpy/math ceil(). We can avoid importing those libraries then.
    # print(scl)
