...
Version b_0.4:
    - Included option to set/limit final time. This allows time cropping of the data   
    - Updated the back end to include t_final set option
    - Added a fail safe check for curve fitting error in the back end
...
# Import backend
# =============================================================================
from ph_bleach_backend_v2_3 import *
# =============================================================================
...
def open_file():
    ...
    if file:
        ...
        frame_count = Image.open(path).n_frames    # get total number of frames in the image using PIL 
        dt = float(ent_dt.get())
        t_final = round(frame_count * dt,2)
                
        ent_tfinal.delete(0,'end')      # remove old text from time limit entry box
        ent_tfinal.insert(ttb.END, t_final)  # insert new text in the time limit entry box
...
def show_plot():
    ...
    print('Current output frame width = %i' % output_frame.winfo_width())
...
def validate_number():
    ...
    try:
        float(ent_tfinal.get())
    except ValueError:
        Messagebox.show_error('Failed to run. Time limit must be a number', 'Input error', parent = output_frame)
        flag = 1    # set flag = 1 to indicate error
...
def run_backend():
    ...
    main_run(path, dt, t_final, exprt_data, obj_detect_each_frame, th_factor, mean_th_fct, blur_order, block_size, obj_size_th_factor, th_type,
...
# Set final time (to crop time data for proper fitting)
lbl_tfinal= ttb.Label(ctrl_frame,text = 'Limit time (min)', bootstyle = 'primary')
ent_tfinal = ttb.Entry(ctrl_frame, bootstyle = 'primary')
...
lbl_tfinal.grid(row = count, column = 1, sticky = 'nw',padx = 10, pady = 10)
ent_tfinal.grid(row = count, column = 1, sticky = 'nw', padx = 10, pady = 40)
