...
test_img_ind = 3                  # test image frame to plot
# =============================================================================
...
def mask_fragment(img):
    ...
    sub_masks_refined = np.zeros([np.shape(im_mask)[0],np.shape(im_mask)[1],N_obj_refined])  # initialize array of image arrays where submasks will be stored
    mask_refined = np.zeros(np.shape(im_mask))   # initialize composite mask from refined sub-masks 
    # py.figure()
